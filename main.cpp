...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	while (Novice::ProcessMessage() == 0)
	{
		...
		if (isActive)
		{
			...
			// 平面との衝突判定
			float distanceToPlane = Func.Dot(plane.normal, ball.position) - plane.distance;
			if (distanceToPlane < ball.radius)
			{
				// 反射処理
				Vector3 reflected = Func.Reflect(ball.velocity, plane.normal);
				ball.velocity = reflected * restitution;
				// 衝突面から少し離す
				ball.position = ball.position + plane.normal * (ball.radius - distanceToPlane);
			}
		}
